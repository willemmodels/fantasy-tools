...
**Seed data is real, not mock.** `prisma/seed.ts` fetches from [nflverse](https://github.com/nflverse/nflverse-data) (a free/public dataset) at seed time and needs network access: active 2026 rosters + real 2025 season stats (joined on `gsis_id`/`player_id`) for names/teams/production, the real 2026 schedule for bye weeks (the week each team doesn't play) and posted sportsbook lines (spread/total → implied points per team, currently only available a few weeks out since books haven't priced the rest of the season), and real contract data (filtered to non-expired `is_active` rows, most-recently-signed kept). The pool is capped per position (`POSITION_CAPS` in `prisma/seed.ts`) to the top ~330 by real 2025 fantasy points, not every active roster spot. There is no free source for 2026 fantasy point projections, per-player prop odds, or a "strength of schedule vs. real opposing defenses" metric — `proj2026`, `upside`, `bustRisk`, and `sos` are this script's own documented estimates computed from the real inputs above (see the comments at the top of `prisma/seed.ts` before changing that formula). `prisma/nflverse.ts` holds the fetch/CSV-parsing helpers (`csv-parse`, since fields like headshot URLs contain unescaped commas that a naive `.split(",")` would break on). `prisma/seed-mock.ts` is the old fully-synthetic generator, kept as an offline fallback (`npm run db:seed:mock`).
...
**Rankings' "Big Baller" toggle** (`ControlsBar` → `useRankingsStore.bigBallerMode`) previews that same preset without needing a configured draft room: `RankingsTable` runs `computeAuctionValues` over the currently-undrafted pool with `BIG_BALLER_STARTUP_ROSTER` at `BIG_BALLER_TEAMS`/`BIG_BALLER_BUDGET` (4 teams / $500, in `src/lib/types.ts` — also what the room-config modal's "Big Baller Startup preset" button sets `teams` to) and renders it as an extra `$` column — it does not resort `order`, since reordering on a format toggle would clobber the user's manual ranking.
